...
# Universal Video Website Setup Script
# This script will automatically detect Debian/Ubuntu and set up the environment
...
detect_distribution() {
    if [[ -f /etc/os-release ]]; then
        ...
        if [[ "$ID" == "ubuntu" ]]; then
            print_success "Detected Ubuntu $DISTRO_VERSION"
            return 0
        elif [[ "$ID" == "debian" ]]; then
            print_success "Detected Debian $DISTRO_VERSION"
            return 0
        else
            print_error "This script supports Ubuntu and Debian only. Detected: $DISTRO_NAME"
            exit 1
        fi
    else
        print_error "Could not detect distribution. Please run on Ubuntu or Debian."
        exit 1
    fi
}
...
install_ffmpeg() {
    ...
    # Install FFmpeg (available in both Ubuntu and Debian repositories)
...
display_final_instructions() {
    ...
    echo "🖥️  Distribution: $DISTRO_NAME $DISTRO_VERSION"
    echo ""
    echo "🔧 Next steps:"
...
main() {
    echo "=========================================="
    echo "🚀 Video Website Universal Setup Script"
    echo "=========================================="
    echo ""
    ...
    echo ""
    echo "Detected system: $DISTRO_NAME $DISTRO_VERSION"
    echo ""
